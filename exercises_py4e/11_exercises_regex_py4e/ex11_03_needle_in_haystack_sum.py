...
#     Actual data: http://py4e-data.dr-chuck.net/regex_sum_847485.txt (There are 75 values and the sum ends with 49)
#
...
if len(fname) < 1:
    ...
    fname = "regex_sum_847485.txt"

with open(fname, encoding="UTF-8") as fh:
    print(sum(int(num) for num in re.findall(search_pattern, fh.read())))

# sum_allnums = 0
# nums_count = 0
# with open(fname, encoding="UTF-8") as fh:
#     for line in fh:
#         digitsinline_list = re.findall(search_pattern, line)
#         if digitsinline_list:
#             nums_count += len(digitsinline_list)
#             for num_str in digitsinline_list:
#                 sum_allnums += int(num_str)

# print(f"{sum_allnums}")
# print(f"Count: {nums_count}")
